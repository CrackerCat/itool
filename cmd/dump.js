...
function dumpModule(name) {
  ...
  var newmodpath = "/private/var/mobile/Media/" + newmodname + ".dump";
...
function copyAppToMedia(app_path, media_path) {
  ...
  defaultManager.createDirectoryAtPath_attributes_(media_path, NULL);
  ...
  for (var i = 0, l = filenames.count(); i < l; i++) {
    ...
    var dest_path = media_path.stringByAppendingPathComponent_(file_name);
    var isDirPtr = Memory.alloc(Process.pointerSize);
    Memory.writePointer(isDirPtr,NULL);
    if (defaultManager.fileExistsAtPath_isDirectory_(file_path, isDirPtr)) {
      defaultManager.copyItemAtPath_toPath_error_(file_path, dest_path, errorPtr);
      // console.log(file_path, "->", dest_path);
      if (Memory.readPointer(isDirPtr) == 1) {
        copyAppToMedia(file_path, dest_path);
      }
    }
  }
}
...
if (ObjC.available) {
  ...
  // start dump
  var appDir = app_path.toString();
  var path = "/private/var/mobile/Media/"+appDir.substr(appDir.lastIndexOf('/')+1);
  copyAppToMedia(app_path, nsstr(path));
  send({app: path});
